...
export default function ChatContainer({ currentChat, socket }) {
  ...
  const handleDeleteMsg = async (message) => {
    try {
      ...
      // Envia una solicitud para eliminar el mensaje en el servidor
  ...
  useEffect(() => {
    arrivalMessage && !deletedMessages.includes(arrivalMessage.id) && setMessages((prev) => [...prev, arrivalMessage]);
  }, [arrivalMessage, deletedMessages]);
            ...
            <div ref={scrollRef} key={uuidv4()}>
              ...
              >
                <div className="content ">
                  <p>{message.message}</p>
                  {message.fromSelf && (
                    <button onClick={() => handleDeleteMsg(message)}>
                      Eliminar
                    </button>
                  )}
                </div>
